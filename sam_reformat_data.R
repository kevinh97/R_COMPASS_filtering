...
combined_tbl <- read.csv("/Users/chanfreaulab/Documents/GitHub/R_COMPASS_filtering/R_COMPASS_filtering/combined_tbl.csv")
  ...
  #Subset each sample to an individual df and combinded those dfs into a list
    ...
    for (i in 1:nrow(master_df)) {
      ...
      for (j in 1:length(list_tbl)){
        ...
        new_colnames <- paste(df_name, colnames(list_tbl[[1]][52:102]), sep = "")
        master_df[i,new_colnames] <<- "FILLER"
        if (length(matched_row) == 102) {
          print(paste(rownames(master_df)[i], "in df", j))
          master_df[i,new_colnames] <<- matched_row[52:102]
          print("Found")
        } else if (length(matched_row) == 0) {
          print("Not Found")
          #print(paste(rownames(master_df)[i], "not in df", j))
